...
def resolve_app_workspace(project_dir: str | Path | None, env_workspace: str | None) -> tuple[Path, str | None]:
    explicit_workspace = project_dir or env_workspace
    workspace_root = Path(explicit_workspace or Path.cwd()).resolve()
    return workspace_root, str(workspace_root) if explicit_workspace else None
