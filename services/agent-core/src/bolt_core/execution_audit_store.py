...
class ExecutionAuditStore:
    ...
    def load(self) -> ExecutionAuditState:
        if not self._path.exists():
            return ExecutionAuditState(queue_items=[], handoff_records=[])
        try:
        ...
        if not isinstance(queue_items, list) or not isinstance(handoff_records, list):
            raise ExecutionAuditStoreError(f"execution audit JSON has invalid lists: {self._path}")
        return ExecutionAuditState(queue_items=queue_items, handoff_records=handoff_records)

    def save_queue_items(self, queue_items: list[dict]) -> None:
        state = self.load()
        self._save(ExecutionAuditState(queue_items=queue_items, handoff_records=state.handoff_records))

    def save_handoff_records(self, handoff_records: list[dict]) -> None:
        state = self.load()
        self._save(ExecutionAuditState(queue_items=state.queue_items, handoff_records=handoff_records))
