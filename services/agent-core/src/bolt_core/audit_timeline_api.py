...
def create_audit_timeline_router(
    ...
    def audit_timeline_summary(closure_id: str | None = None) -> dict:
        """获取审计时间线摘要。只读。

        可传入 closure_id 过滤。未传时返回最近 20 条时间线事件。
        示例：GET /audit-timeline?closure_id=xxx
        """
        if closure_id:
        ...
        return {"events": all_events[:20], "total": len(all_events), "closure_id": None}
