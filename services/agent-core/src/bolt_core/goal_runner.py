...
class GoalRunner:
    def __init__(self, step_fn, evidence_log: EvidenceLog, completion_check_fn=None) -> None:
    ...
    def run(self, goal: Goal) -> GoalRunnerResult:
        if goal.status not in (GoalStatus.RUNNING, GoalStatus.PENDING):
            return GoalRunnerResult(goal.status, "goal not in runnable state", goal.step_count)
        ...
        while True:
            ...
            if step_num > goal.max_steps:
                return GoalRunnerResult(GoalStatus.STOPPED, "max_steps exceeded", step_num - 1)

            if total_cost >= goal.max_cost:
                return GoalRunnerResult(GoalStatus.STOPPED, "max_cost exceeded", step_num - 1)

            if elapsed >= goal.max_wall_time:
                return GoalRunnerResult(GoalStatus.STOPPED, "max_wall_time exceeded", step_num - 1)
            ...
            if step_result.get("status") == "pending_permission":
                return GoalRunnerResult(GoalStatus.PAUSED, "paused for permission", step_num)

            if step_result.get("status") == "failed":
                continue

            if self._completion_check_fn and self._completion_check_fn(goal, self._evidence_log):
            ...
            elapsed = time.monotonic() - start_time
            if elapsed >= goal.max_wall_time:
                return GoalRunnerResult(GoalStatus.STOPPED, "max_wall_time exceeded", step_num)
