...
function apiFixture(): TaskClosurePanelApi {
  return {
    ...
    bindTaskClosureRun: vi.fn().mockImplementation((_id, runId) => Promise.resolve(closure({ run_id: runId }))),
    bindTaskClosureGoal: vi.fn().mockImplementation((_id, goalId) => Promise.resolve(closure({ goal_id: goalId }))),
...
describe('TaskClosurePanel', () => {
  ...
  it('显示任务闭环标题', () => {
    render(<TaskClosurePanel workspace={workspace} api={apiFixture()} />);
  ...
  it('workspace 为空时禁用创建', () => {
    render(<TaskClosurePanel workspace="" api={apiFixture()} />);
  ...
  it('workspace 为空时禁用绑定按钮', async () => {
    ...
    render(<TaskClosurePanel workspace="" runId="run_43" goalId="goal_43" api={api} />);
    expect(screen.getByRole('button', { name: '创建闭环任务' })).toBeDisabled();
  });
  ...
  it('点击创建调用 createTaskClosure', async () => {
    ...
    render(<TaskClosurePanel workspace={workspace} fetcher={fetcher} runId="run_42" goalId="goal_42" api={api} onClosureChange={onClosureChange} />);

    await createClosure(api);

    expect(api.createTaskClosure).toHaveBeenCalledWith({
      objective: '修复拼写',
  ...
  it('创建后显示中文状态标签', async () => {
    const api = apiFixture();
    render(<TaskClosurePanel workspace={workspace} api={api} />);
  ...
  it('记录验证命令调用 addClosureEvent', async () => {
    const api = apiFixture();
    const { container } = render(<TaskClosurePanel workspace={workspace} fetcher={fetcher} api={api} />);
    await createClosure(api);
    ...
    await vi.waitFor(() => expect(api.addClosureEvent).toHaveBeenCalledWith('cl_42', { type: 'command', command: 'pnpm test', result: '335 passed' }, fetcher));
  ...
  it('记录审查摘要调用 addClosureReview', async () => {
    ...
    await vi.waitFor(() => expect(api.addClosureReview).toHaveBeenCalledWith('cl_42', { summary: '全部通过', passed: true }, fetcher));
    await vi.waitFor(() => expect(container).toHaveTextContent('审查摘要：全部通过'));
  });

  it('点击绑定当前运行调用 bindTaskClosureRun', async () => {
    const api = apiFixture();
    render(<TaskClosurePanel workspace={workspace} fetcher={fetcher} runId="run_43" api={api} />);
    await createClosure(api);

    fireEvent.click(screen.getByRole('button', { name: '绑定当前运行' }));

    await vi.waitFor(() => expect(api.bindTaskClosureRun).toHaveBeenCalledWith('cl_42', 'run_43', fetcher));
    expect(await screen.findByText('已绑定运行：run_43')).toBeInTheDocument();
  });

  it('点击绑定当前目标调用 bindTaskClosureGoal', async () => {
    const api = apiFixture();
    render(<TaskClosurePanel workspace={workspace} fetcher={fetcher} goalId="goal_43" api={api} />);
    await createClosure(api);

    fireEvent.click(screen.getByRole('button', { name: '绑定当前目标' }));

    await vi.waitFor(() => expect(api.bindTaskClosureGoal).toHaveBeenCalledWith('cl_42', 'goal_43', fetcher));
  ...
  it('显示自动同步状态提示', async () => {
    ...
    for (const item of cases) {
      ...
      const { unmount } = render(<TaskClosurePanel workspace={workspace} api={api} />);
      await createClosure(api);
  ...
  it('不调用 runAgentLoop 或 approvePermission', async () => {
    ...
    render(<TaskClosurePanel workspace={workspace} fetcher={fetcher} api={api} />);
  ...
  it('点击评估完成度调用 update API', async () => {
    ...
    await vi.waitFor(() => expect(api.updateTaskClosureAssessment).toHaveBeenCalledWith('cl_42', fetcher));
  });
