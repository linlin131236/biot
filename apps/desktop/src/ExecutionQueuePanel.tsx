...
interface Props { baseUrl: string; closureId?: string | null; fetcher?: Fetcher; api?: ExecutionQueuePanelApi; onApprovedItemChange?: (itemId: string | null) => void; }
...
export default function ExecutionQueuePanel({ baseUrl, closureId, fetcher, api, onApprovedItemChange }: Props) {
  ...
  return <aside className="panel"><h2>安全执行队列</h2><button type="button" onClick={propose}>生成待处理动作</button><h3>待处理动作</h3>{items.length ? items.map(item => <QueueItem key={item.id} item={item} update={update} api={call} baseUrl={baseUrl} fetcher={fetcher} onApprovedItemChange={onApprovedItemChange} />) : <p>需要人工处理</p>}{error ? <p>{error}</p> : null}</aside>;
}

function QueueItem({ item, update, api, baseUrl, fetcher, onApprovedItemChange }: { item: ExecutionQueueItem; update: (a: () => Promise<ExecutionQueueItem>) => void; api: ExecutionQueuePanelApi; baseUrl: string; fetcher?: Fetcher; onApprovedItemChange?: (itemId: string | null) => void }) {
  const canCompletePending = item.status === 'pending' && (item.risk === 'read_only' || item.kind === 'manual_review');
  return <div className="stack"><strong>{item.title}</strong><span>{item.description}</span><span>风险等级：{riskLabels[item.risk]}</span><span>{statusLabels[item.status]}</span>{item.command ? <code>命令建议：{item.command}（不执行命令）</code> : null}<div className="actions">{item.status === 'pending' ? <><button type="button" onClick={() => update(() => api.approveExecutionQueueItem(baseUrl, item.id, fetcher))}>批准</button><button type="button" onClick={() => update(() => api.rejectExecutionQueueItem(baseUrl, item.id, '用户拒绝', fetcher))}>拒绝</button></> : null}{item.status === 'approved' ? <button type="button" onClick={() => onApprovedItemChange?.(item.id)}>用于交接</button> : null}{canCompletePending || item.status === 'approved' ? <button type="button" onClick={() => update(() => api.completeExecutionQueueItem(baseUrl, item.id, '用户已完成', fetcher))}>标记完成</button> : null}{item.status === 'approved' ? <button type="button" onClick={() => update(() => api.failExecutionQueueItem(baseUrl, item.id, '用户标记失败', fetcher))}>标记失败</button> : null}</div></div>;
}
