...
function apiFixture() {
  return {
    ...
    executionHandoff: { fetchExecutionHandoffs: vi.fn().mockResolvedValue([]), fetchExecutionAuditTimeline: vi.fn().mockResolvedValue([]), fetchExecutionAuditDiagnostics: vi.fn().mockResolvedValue([]), fetchExecutionAuditIntegrity: vi.fn().mockResolvedValue([]), fetchReleaseReadiness: vi.fn().mockResolvedValue({ ready: true, checks: [], blockers: [], warnings: [] }), fetchLocalReleaseChecklist: vi.fn().mockResolvedValue({ ready: true, items: [], blockers: [], warnings: [], next_step: '', disclaimer: '' }), createExecutionHandoff: vi.fn(), completeExecutionHandoff: vi.fn(), failExecutionHandoff: vi.fn(), requestExecutionHandoffPermission: vi.fn() },
  };
}
