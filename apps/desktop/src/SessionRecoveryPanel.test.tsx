...
describe('SessionRecoveryPanel', () => {
  it('shows empty state', async () => {
    render(<SessionRecoveryPanel api={{ fetchSessionRecovery: vi.fn().mockResolvedValue(empty) }} />);
    await waitFor(() => expect(screen.getByText(/暂无暂停/)).toBeTruthy());
  });

  it('shows paused tasks', async () => {
    render(<SessionRecoveryPanel api={{ fetchSessionRecovery: vi.fn().mockResolvedValue(full) }} />);
